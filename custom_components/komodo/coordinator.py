    ...
    async def _async_update_data(self):
        ...
        async with asyncio.timeout(10):
            tasks = [
                ...
                # Mirrors upstream commit 1c3067ff.
            ...
            # Per-container stats (Option B). listAllStackServices only exists
            # on newer komodo-api (2.3 beta+); the pinned 2.2.0b3 exposes the
            # per-stack listStackServices, which returns the same service
            # container (with stats) for a single stack. One call per stack.
            stack_services = await asyncio.gather(
                *(
                    self.my_api.read.listStackServices(
                        ListStackServices(stack=stack_id)
                    )
                    for stack_id in data.stacks
                ),
                return_exceptions=True,
            )
            for stack_id, response in zip(data.stacks, stack_services):
                if isinstance(response, Exception):
                    _LOGGER.error(
                        "Error fetching services for stack %s", stack_id,
                        exc_info=response,
                    )
                else:
